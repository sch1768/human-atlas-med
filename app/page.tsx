...
import { Activity, ArrowUpRight, BoxSelect, ChevronDown, ChevronLeft, ChevronRight, ChevronUp, EyeOff, Focus, Info, Layers3, Pause, RotateCcw, RotateCw, Search, Undo2, X } from 'lucide-react';
...
import { getDisplayName, getHanjaName, matchesSearchTerm, getKoreanTerms, isVascularAbbreviation } from './korean-anatomy';
...
export default function Home() {
  ...
  const results = useMemo(() => {
    ...
    if (!term) return [];
    const abbreviationQuery = isVascularAbbreviation(term);
    const matchedConcepts = allConcepts.filter(c => matchesSearchTerm(c.name, term) || (!abbreviationQuery && c.id.toLowerCase().includes(term)));
    const conceptNames = new Set(allConcepts.map(c => c.name.toLowerCase())), matchedParts: Concept[] = [];
    atlas.parts.forEach(p => { if (!conceptNames.has(p.name.toLowerCase()) && (matchesSearchTerm(p.name, term) || (!abbreviationQuery && p.id.toLowerCase().includes(term)))) { matchedParts.push({ id: p.id, name: p.name, elements: [p.id] }); } });
  ...
  return <main className={`studio ${panel ? `panel-open panel-${panel}` : ''} ${details ? 'details-open' : ''}`}>
    ...
    {panel === 'search' && <section className="search-panel glass" aria-label="Find anatomy">
      ...
      <div className="search-input-box">
        ...
        }} placeholder="넙다리뼈, femur, SMA, IVC, LAD…" aria-label="Search named anatomical structures" />
      ...
      {!query.trim() ? (
        <div className="quick-picks-section">
          ...
          <p className="search-note">영문(Femur), 신용어(넙다리뼈), 구용어(대퇴골), 주요 혈관 약어(SMA, IVC, LAD)로 검색해 보세요.</p>
        </div>
      ) : (
